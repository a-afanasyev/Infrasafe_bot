...
# AUD3-06: requests.py разбит на пакет handlers/requests/ — сканируем все модули.
HANDLER_DIR = (
...
def _relpath(p: Path) -> str:
    return f"uk_management_bot/handlers/requests/{p.name}"
...
def collect_orm_sites(files: list[Path] = PACKAGE_FILES) -> set[tuple[str, str]]:
    """→ {(relpath, signal)} прямого ORM во всех модулях requests-пакета."""
...
def test_requests_handler_has_no_next_get_db():
    ...
    assert not matches, (
        "Найден legacy `next(get_db())` в handlers/requests/ "
