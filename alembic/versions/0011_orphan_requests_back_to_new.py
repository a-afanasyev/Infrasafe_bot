...
⚠️ Условие ШИРЕ, чем «продукты групповой диспетчеризации»: под него попадает
любая заявка «В работе» без исполнителя, включая принятые менеджером старым
пустым `MANAGER_ASSIGN` («менеджер взял, исполнителя выберет потом»). Так и
задумано — инвариант не различает происхождение. Номера печатаются в лог, чтобы
их можно было сверить глазами до/после применения.

⚠️ Realtime и outbox эта миграция НЕ эмитит: открытые дашборды и внешний
потребитель (InfraSafe-вебхук) узнают о новом статусе только со следующим
событием по заявке. Для девяти заявок это осознанный компромисс — тянуть
эмиссию событий в миграцию дороже и рискованнее, чем один раз обновить доску.
...
def upgrade() -> None:
    ...
    # `status_version` бампается вручную: канон делает это на каждом переходе
    # (`workflow_runner._apply_*`), и строка со «старой» версией после смены
    # статуса выглядела бы для оптимистичной блокировки неизменённой.
    bind.execute(
        sa.text(
