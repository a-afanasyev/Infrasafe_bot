...
def merge_additional_keys():
    ...
    for section, keys in ru_additional.items():
        if section not in ru_data:
            ru_data[section] = {}
        # Исправляем ошибку с обновлением словаря
        if isinstance(keys, dict):
            for key, value in keys.items():
                ru_data[section][key] = value
        else:
            print(f"Предупреждение: секция '{section}' имеет неверный формат: {type(keys)}")
    
    # Создаем узбекские ключи на основе русских
    uz_additional = {}
    for section, keys in ru_additional.items():
        if section not in uz_additional:
        ...
        if section == "buttons":
            ...
            uz_additional[section] = {
                ...
                "status_transitions": {
                    ...
                    "Bajarilgan": ["Qabul qilingan", "Ishda", "Bekor qilindi"],
                    "Qabul qilingan": [],
                    "Bekor qilindi": []
                }
            }
    
    # Объединяем узбекские ключи
    for section, keys in uz_additional.items():
