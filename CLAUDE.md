...
- **Миграции/деплой**: история Alembic сжата в baseline `001` + seed `002` (PRC-05, 2026-07-10); оба прода на `alembic @003`. CI-дрейф-гейт: `alembic upgrade head` + `alembic check`. ⚠️ `alembic stamp --purge` — ТОЛЬКО для разового пере-baseline (следующий squash), НЕ рутинный деплой.
  - **⚠️ PR-7 (F-01) закодирован, но НЕ раскатан на прод** (2026-07-15): `scripts/entrypoint-api.sh` больше НЕ гоняет `alembic upgrade head` — это теперь read-only preflight (`uk_management_bot/dbops/db_preflight.py`), миграции вынесены в отдельный one-shot `migrate`-профиль (`docker compose run --rm --name uk-migrate migrate`, `entrypoint-migrate.sh`). Целевой рутинный деплой ПОСЛЕ DBA-окна: `git pull && docker compose build api access-api app && docker compose run --rm --name uk-migrate migrate && docker compose up -d api access-api app`. До DBA-окна (Шаг 0 inventory → `provision-roles` → ownership transfer → `migrate` → smoke, см. план PR-7) на проде НИЧЕГО из этого не выполнять — `docker compose up -d api` на текущем проде тихо не сломается (env_file на `.secrets/roles/.env.api` упадёт явной ошибкой «file not found» ДО старта контейнера), но реальный cutover требует полного рантайма, не одной строки.
