...
`SELECTABLE_CATEGORY_KEYS`); источник (`llm|keyword|manual|default`) уходит в
`AuditLog.details.category_source` записи `request.created_from_group`
(`default` — фолбэк тег-режима в «Другое», когда модель сказала «не заявка»
и словарь молчит: это не вердикт LLM и в метрику модели не идёт).

**ПДн и LLM (решение владельца 2026-09-23, A9-P3-5).** Текст сообщения из
группы уходит внешнему провайдеру (Anthropic) только после маскирования
телефонов (до усечения текста до 2000 символов): `classifier.mask_phones`
заменяет номера в узбекской группировке — необязательный `+998`/`998`, код
оператора из 2 цифр (можно в скобках), затем 3-2-2 с пробелами/дефисами или
слитно (`+998 (90) 123-45-67`, `90 123 45 67`, `901234567`) — на `[PHONE]`.
Адрес и номер квартиры НЕ маскируются — они нужны для `address_hint`; не
трогаются и номера заявок (`250923-001`), суммы, даты. Иностранные форматы
(`+7 …`, `8 990 …`) шаблоном не покрыты — в группах ЖК не встречаются.

**Устойчивость (тег-режим).** Тег — явное намерение автора, поэтому и сбой
классификатора (`PROCESSING_ERROR`, BUG-192), и отказ LLM-лимитера группы
(A9-P2-6) отвечают автору `classifier_unavailable`; при отказе лимитера
dedup-метка `gint:seen:*` снимается, ответ — не больше одного в чат за окно
лимитера (`gint:busy:{chat}`, 60 с) и не чаще раза в 5 минут одному автору
(`gint:busy:{chat}:{author}`). Разовый сетевой сбой/5xx/429 повторяется один
раз: jitter-пауза, на 429 — `retry-after`; весь вызов укладывается в общий
дедлайн 2 × `GROUP_INTAKE_LLM_TIMEOUT` (пауза и вторая попытка — из остатка),
повтор проходит тот же лимитер группы. Теги матчатся целым словом
(`#заявкам` — не тег).

**Фото из группы (A9-P2-5).** `file_id` фото принадлежит групповому боту и
основным ботом не открывается — в `media_files` он не пишется. Фото
копируется в media-service, в заявку уходит маркер `{"media_id", "type"}`
(`services/request_media_markers.py`, тот же формат, что у загрузки из
дашборда); исполнитель и менеджер получают его байтами из media-service.
Если media-service недоступен, заявка создаётся без фото и в группу об этом
не пишется (шум) — только лог с номером заявки.
