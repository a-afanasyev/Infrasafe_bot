...
| **`ShiftAssignmentService`** | `services/shift_assignment_service.py:911` | **Фасад.** Оркестрация автоназначения исполнителей на смены (`auto_assign_executors_to_shifts`), переназначение при отсутствии, аудит, уведомления. Делегирует в подклассы. | `auto_assign_executors_to_shifts(shifts, force_reassign)` (`:939`), `reassign_on_absence` (`:1263`), `balance_executor_workload` (`:1099`, делегат) | `Dict` результатов / `ExecutorScore` |
