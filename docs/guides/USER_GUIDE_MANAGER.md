...
- **Назначение/переназначение исполнителя** — drag на «В работе» или кнопка в
  карточке: список показывает **только специалистов, подходящих под категорию
  заявки** (универсалы — везде); вариант «Дежурный» подбирает сервер (409 с
  причиной, если на смене никого). Заявку можно адресовать группе — она
  останется «Новая» и попадёт в пул дежурных.
- **Staff-заявки из групп** (в карточке видно происхождение): у них
  менеджерская приёмка — «Подтвердить» сразу закрывает заявку в «Принято»,
  этапа жителя нет.
...
- В шапке карточки — **Исправить ФИО** (`EditFullNameModal`): сотрудник вводит
  ФИО сам после приглашения, и опечатку правит менеджер. Поле одно, ФИО
  вводится строкой — так же, как при регистрации. Та же кнопка есть в карточке
  жителя `/dashboard/residents/:id`. ФИО менеджера и админа не правится: свой
  профиль они редактируют сами (`services/users/rename.py`).
...
### A.11. Группы (Group Intake) — `/dashboard/groups`

Реестр Telegram-групп автоматического приёма заявок (`GroupsPage.tsx`):

- Добавление группы: chat_id (формат `-100…`), тип **жительская / staff**,
  активность.
- Переключатель **«По тегу / Все сообщения»** (тег-режим): в тег-режиме бот
  обрабатывает только сообщения с `#заявка`/`#ариза`/`#ariza`/`#zayavka` — ноль реакции и ноль
  LLM-затрат на остальное.
- Staff-группы: писать могут только approved-сотрудники; обязательное фото;
  заявка создаётся с менеджерской приёмкой (см. A.1).
- После добавления группы: пригласить бота в группу и закрепить сообщение с
  правилами («нет номера — нет заявки»).

Бизнес-процесс целиком — [../product/BUSINESS_PROCESSES.md §5](../product/BUSINESS_PROCESSES.md).

### A.12. Учёт ресурсов — `/dashboard/resource-accounting`

Каталог объектов/счётчиков, отчётные периоды, ввод и проверка показаний,
выгрузки. Раздел за build-флагом `VITE_RESOURCES_ENABLED`; вход прозрачный
(s2s-тикет). Капабилити контролёра выдаётся в карточке сотрудника
(«Ввод показаний»).
...
- **Исправление ФИО** — кнопка «✏️ Исправить ФИО» в карточке жителя и «📝 ФИО»
  в карточке сотрудника ведут в один общий флоу (`handlers/user_rename.py`).
  ФИО вводится одной строкой; правка пишется в `audit_logs` действием
  `user_renamed` со старым и новым значением.
