...
- `receive_shared_contact` ловит контакт только без FSM-состояния. Если у
  пользователя в боте висит незавершённый диалог (например, создание заявки),
  контакт из TWA никто не примет, и TWA уйдёт в 30-секундный таймаут с кнопкой
  «Проверить ещё раз». Для регистрации маловероятно; лечится `/start` в боте.
