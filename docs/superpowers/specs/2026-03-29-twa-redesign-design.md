...
| `GET /requests/{number}` | Любой видит любую заявку | Проверка: user_id, apartment_id (approved resident), executor (через RequestAssignment ИЛИ executor_id), или manager role |
| `GET /requests/{number}/comments` | Нет проверки участия | Проверка: пользователь — участник заявки (owner, apartment resident, assigned executor, manager) |
