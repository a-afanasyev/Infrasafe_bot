...
сообщения с `#заявка`/`#ариза`/`#ariza`/`#zayavka` — ноль LLM-вызовов и ноль реакций на всё
...
Статусы в БД — русские строки; канон — `utils/constants.py`. Все переходы совершаются ТОЛЬКО через канонический workflow
...
- FR-2.3 Классификатор: Anthropic structured outputs (модель из env, дефолт
  ...
  заявка», логируется маркером; разовый сетевой сбой/таймаут повторяется один
  раз, в тег-режиме при итоговом сбое автор получает просьбу повторить.
...
| Тег-режим | Группа обрабатывает только сообщения с `#заявка`/`#ариза`/`#ariza`/`#zayavka` |
