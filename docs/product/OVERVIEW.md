...
`uk_management_bot/utils/enums.py:9` (`UserRole`). Устаревшее одиночное поле
...
- **Telegram-группы (Group Intake)** — отдельный бот молча читает
  ...
  заявку с номером. Поддерживается тег-режим (`#заявка`/`#ариза`/`#ariza`/`#zayavka`) и
...
сообщений с `#заявка`/`#ариза`/`#ariza`/`#zayavka`. Реестр групп — страница «Группы» дашборда.
...
Канонический перечень статусов — `uk_management_bot/utils/constants.py`
(`REQUEST_STATUS_*`; в БД хранятся русскоязычные строки). Новая заявка создаётся со статусом
