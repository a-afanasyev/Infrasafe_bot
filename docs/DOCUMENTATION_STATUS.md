...
> 23 файла. Пути в таблицах ниже указаны как были на момент аудита 2026-07-06;
> сами файлы теперь в архиве, там же таблица «что устарело и куда смотреть
> вместо этого». Причина переноса: пометка 🔴 в этой матрице не защищает — файл
> открывают по имени, а не через индекс. Поэтому статусы 🔴/⚫ в корне `docs/`
> впредь не оставляем: либо правим документ, либо архивируем.
>
> Не перенесены осознанно: `uk_management_bot/README.md` и `frontend/README.md`
> (🔴, но это README сервисов — их место в дереве кода, переписывать надо, а не
> архивировать), `DEPLOYMENT_CHECKLIST.md` и `DOCKER_SETUP.md` (🟡/«исправлено»).
