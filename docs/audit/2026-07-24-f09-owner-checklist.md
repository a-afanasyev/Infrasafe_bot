...
2. **www.profk.uz — ЗАКРЫТО.** Выбран вариант удаления алиаса: CNAME `www`
   ...
   невозможен.
3. **Edge-nginx — ЗАКРЫТО, подтверждено полностью (2026-07-25).**
   `frame-ancestors 'self' https://web.telegram.org` в CSP `/uk/*` — live на
   ОБОИХ доменах (profk.uz + infrasafe.uz). Handshake-щит на `/uk/ws/v2/`
   (`limit_conn 10/IP + limit_req 10r/m burst 10`) — подтверждён честным
   тестом: 14 одновременных настоящих WS-handshake → ровно 10 открыто,
   4 отклонено (429 rate-limit + 503 conn-limit). Методологическая грабля
   для будущих проверок: зона матчит реальные WebSocket-upgrade запросы —
   burst голых HTTP GET на тот же путь проходит мимо лимита (все 404 от
   бэкенда) и НЕ является валидным тестом щита.

   Попутно проверено: allowlist-префикс `/uk/api/v2/auto-manager-config`
   работает на обоих доменах (401 — авторизация, не 404-заглушка edge).

**Итог: все три пункта чек-листа исполнены владельцем и подтверждены нашей
независимой re-проверкой. F-08/F-09 закрыты целиком.**
