...
> **⚠️ СТАТУС НА 2026-06-25 — план почти полностью исполнен. См. раздел «1.5 Статус исполнения».**
> Волны 0–4 закрыты (PR-1…PR-35, PR-40 — в проде). Остаток — практики/тест-инфра (волна 5)
> и несколько осознанно отложенных пунктов. Тело плана ниже (§2 и далее) — исходное проектное,
> отдельные PR-блоки не помечались ✅ по ходу; источник истины по статусу — §1.5 и бэклог.
...
## 1.5 Статус исполнения (обновлено 2026-06-25)

Проверено по git-истории (feature-коммиты/merge-PR) + маркерам бэклога. **Из ~39 PR закрыто ~36.**

| Волна | PR | Статус | Подтверждение |
|---|---|---|---|
| **0** | PR-1…PR-5 | ✅ done, прод | PR #59-63 |
| **1** | PR-6a/6b/7 | ✅ done, прод | PR #64/68/66 |
| **2** | PR-8…PR-11 | ✅ done, прод | PR #67/69/70/71 |
| **2.5** | actuality-пасс | ✅ done | без кода |
| **3** | PR-12…PR-25, PR-40 | ✅ done, прод | PR #72-93 (вкл. кластеры address/FE/bot, FE-121) |
| **4** | PR-26…PR-35 | ✅ done, прод | PR #117-131 (ARCH-012/05/01/07/02, DB-60, FE-09, DB-34) |
| **5** | PR-36 (PRAC-01+FU) | ✅ done | PR #102/106-108/112/113 (ruff-гейт whole-scope) |
| **5** | PR-37 (PRAC-02/ARCH-115/SEC-09) | ✅ done 2026-06-25 | README + branch-policy + known-constraints |
| **5** | PR-38 (DEAD-15 + NICE-114) | ⬜ **открыт** | перенос колоцированных тестов + test-conventions |
| **5** | PR-39 (TEST-071) | 🟡 **частично** | e2e-скелет есть (`tests/e2e/specs/login-flow.spec.ts`), нет seed-фикстуры → login-redirect тест skipped |

**Реально оставшаяся работа:**
- **PR-38 / DEAD-15** (P3) — перенести ~30 колоцированных тестов в `uk_management_bot/tests/`. ⚠️ landmine-гейт: не делать `tests/api` пакетом, `--import-mode=importlib`, `--collect-only` до/после не должен уменьшиться.
- **PR-38 / NICE-114** (P3) — стандартизировать test-conventions (`@pytest.mark.unit/integration`, единый asyncio-стиль, `from __future__ import annotations`) + doc.
- **PR-39 / TEST-071** (P2) — seed-скрипт `scripts/seed_e2e_user.py` + `tests/e2e/.env.example`, расскипать `successful login redirects`.
- **PR-35 / TEST-068** (P2, 🟡 ongoing) — frontend vitest-ratchet продолжается, 80% ещё не достигнуты.
- **REFACTOR-113** (PR-36, P3) — G004 lazy-logging sweep (eager f-string → `%s`); ruff-гейт сейчас E+F, G не включён.
- **CODE-04** (PR-29, P3, partial) — остаточный sweep `next(get_db())`→`session_scope()` вне уже выправленных файлов.
- **DB-049** (PR-31, отложено) — `users.roles` остаётся `text` со строковым LIKE-матчингом (DB-060 — дроп legacy `role` — сделан); конверсия в `jsonb`+GIN не делалась, на пилоте не нужна.

**✅ Marker-hygiene выполнен 2026-06-25** (верификация 2 агентами против кода/git/CI). Зачёркнуты подтверждённые DONE-в-коде: **ARCH-01** (scoped — 3 god-хендлера, PR-29; остаток в прочих хендлерах = CODE-04/AUD3-07), **ARCH-02** (PR-32), **ARCH-05** (PR-27/28), **FE-12** (PR-35 Ph2), **AUD3-11** (PR-12 UTC), **AUD3-18** (form-data 4.0.6). Помечены 🔵 DEFERRED (8): ARCH-06/010/107/108, SEC-115, DB-112, DB-049, AUD3-17/32. **Подтверждены ОТКРЫТЫМИ (НЕ drift, реальная работа):** **NICE-078** (дедуп `_parse_user_roles` так и НЕ сделан — два парсера живут параллельно; ранее ошибочно числился в drift), **REFACTOR-113** (ruff G004 не включён, ~1322 eager f-string), **CODE-04** (PARTIAL — 95 `next(get_db())` в неохваченных файлах). AUD3-кластер: ~21 реально открытых P2/P3 (god-файлы AUD3-06, broad-except AUD3-10/27, redis/bot timeouts AUD3-08/09, set-password current_password AUD3-16, mypy/coverage в CI AUD3-03/04 и др.) — низкоприоритетный тех-долг, не блокирует.
