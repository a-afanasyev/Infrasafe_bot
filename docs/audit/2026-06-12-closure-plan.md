...
| Действие | Пункты |
|---|---|
| Зачеркнуть с обоснованием (после утверждения владельцем) | OPS-112, BUG-BOT-038, NICE-077 (после контрольного grep), NOTE-S-M7, NICE-072/073/074, DB-055, DB-103, REFACTOR-026 |
| Marker-hygiene (§6) | FIX-002, FIX-004 — верифицировать прод-факты, обновить тела |
| Переписать тело → PR-18 | FIX-008 (ре-спецификация: de-async 12 read-методов) |
| Ручной прод-SQL (после подтверждения) | BUG-BOT-017 (junk-шаблоны), MGR-09 (дубль здания) |
| Предусловие PR-10 | ssh-сверка прод-копии docker-compose.yml |
...
## 5. DECISION — требуют выбора владельца

| Пункт | Варианты |
|---|---|
| ARCH-106 | Платформа секретов: Doppler / 1Password / AWS-SM → после выбора станет PR (1–2 d) |
| DEAD-06 + FE-121 | (а) реализовать UI смены пароля (мини-PR, волна 3) и оставить эндпоинт; (б) удалить `set-password` и закрыть оба |
| BUG-BOT-040 | MetricsManager достижим из живого `shift_planning_service`: (а) отключить/удалить путь; (б) починить async-sync; (в) зафиксировать accepted-risk с обоснованием |
| BUG-BOT-017 | Подтвердить ручную прод-чистку junk-шаблонов |
| MGR-09 | Мердж/удаление дубля здания — вариант SQL |
| Untracked `tests/e2e/`, `uz-board.yml` | Закоммитить или удалить (влияет на PR-39) |
