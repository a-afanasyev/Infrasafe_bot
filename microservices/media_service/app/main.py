...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # Инициализация observability service
        ...
        import asyncio
        asyncio.create_task(cleanup_metrics_task(observability))
    ...
    try:
        ...
        logger.info("Observability service closed successfully")
    except Exception as e:
        logger.error(f"Error closing observability service: {e}")


async def cleanup_metrics_task(observability):
    """
    ...
    while True:
        try:
            ...
            await observability.cleanup_old_metrics(max_age_hours=24)
...
app.add_middleware(RateLimitingMiddleware, redis_url=getattr(settings, 'redis_url', 'redis://localhost:6379'))
...
async def log_and_track_requests(request: Request, call_next):
    ...
    try:
        ...
        await observability.metrics.record_histogram("request_duration_ms", duration_ms, tags)
        await observability.metrics.record_counter("requests_total", 1, tags)

        # Track success/error rates
        if 200 <= response.status_code < 400:
            await observability.metrics.record_counter("requests_success_total", 1, tags)
        else:
            await observability.metrics.record_counter("requests_error_total", 1, tags)
