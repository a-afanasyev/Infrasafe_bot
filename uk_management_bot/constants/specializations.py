...
def normalize_specialization(value: object, *, side: str = "have") -> set[str]:
    ...
    Асимметрия сохранена. `hvac` — единственный токен, который разворачивается
    в два значения, и сторона «требуется» его схлопывает по двум причинам:

    * скалярные поля (`requests.assigned_group`,
      `request_assignments.group_specialization` — `String(100)`) физически не
      вмещают множество, а именно они и есть «требование» в предикате доступа;
    * исторический вид смены/шаблона: у них `hvac` означал ОДНУ позицию, и
      разворачивать её задним числом — молча менять смысл существующих строк.

    ⚠️ Прежнее обоснование («расширение в „требуется“ СУЖАЕТ пул, потому что
    нужны ВСЕ значения») с BUG-166 больше не действует: подбор перешёл на
    семантику ЛЮБАЯ (`matches_required_specs`), и расширение требования там,
    наоборот, расширило бы пул. Поведение оставлено байт-в-байт — менять
    трактовку данных заодно с семантикой предиката нельзя.
