...
class Action(str, Enum):
    ...
    # Адресовать заявку ГРУППЕ-специализации, НЕ меняя статус (решение
    # владельца 2026-08-17: «В работе» ⟺ есть исполнитель). Раньше группу
    # назначали теми же двумя действиями выше, и заявка уезжала в «В работе»
    # с пустым `executor_id`: если никто из дежурных её не брал, она висела
    # ничьей — так на продах накопилось девять таких заявок. Теперь группа
    # означает «показать дежурным нужной специализации», заявка остаётся
    # «Новая», а «В работе» наступает только с появлением человека.
    #
    # Одно действие на систему и менеджера сознательно: вопрос один и тот же
    # («адресовать группе»), различаются только принципал и аудит, а разводить
    # его на два action'а значило бы держать две копии одной семантики.
