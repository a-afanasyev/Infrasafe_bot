...
async def _load_yards(telegram_id: int) -> YardsView:
    view = await card.run_unit(run_db, lambda s: load_yards(s, telegram_id), "список дворов")
...
async def handle_yards(callback: CallbackQuery, state: FSMContext, language: str = "ru") -> None:
    ...
    if view.verdict != OK:
        ...
        return
    await state.clear()
    await card.edit(callback, _yards_text(view, language), yards_keyboard(view.yards, language))
...
async def handle_yard(callback: CallbackQuery, language: str = "ru") -> None:
    ...
    yard_id = card.parse_int(match.group("id")) if match else None
    if yard_id is None:
    ...
    await card.edit(callback, text, buildings_keyboard(view.buildings, language))
...
async def handle_building(callback: CallbackQuery, language: str = "ru") -> None:
    ...
    building_id = card.parse_int(match.group("id")) if match else None
    ...
    await card.edit(callback, text, elevators_keyboard(view.elevators, view.yard_id, language))
...
async def handle_cancel(callback: CallbackQuery, state: FSMContext, language: str = "ru") -> None:
    ...
    if elevator_id is not None:
        await card.answer_card(run_db, callback.message, callback.from_user.id, int(elevator_id), language)
