...
async def handle_view_request(callback: CallbackQuery, state: FSMContext):
    ...
    try:
        ...
        if active_role == "executor":
            ...
            if request.status in ("Новая", "В работе") and request.executor_id is None:
                # Непривязанная group-заявка → «Взять» (а не Выполнена/Закуп —
                # работать может только взявший; авторизацию взятия проверяет
                # EXECUTOR_CLAIM в claim-callback).
                # Инвариант «В работе ⟺ есть исполнитель» (2026-08-17): такие
                # заявки теперь лежат в «Новой» — групповое назначение статус не
                # двигает. «В работе» здесь оставлено для legacy-строк: до
                # миграции они висели именно там, и сузить условие значило бы
                # отобрать кнопку у заявки, которую ещё можно взять.
