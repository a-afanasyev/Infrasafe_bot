...
async def get_kanban(
    ...
    AUD5-APIFE-3: раньше это был ОДИН запрос `ORDER BY created_at DESC LIMIT 500`
    на всю доску. «Принято»/«Отменена» копятся вечно, поэтому после 500 строк с
    доски пропадали самые старые АКТИВНЫЕ карточки — ровно та работа, которую
    менеджер обязан видеть, — а `count` считался по обрезанному набору и врал.

    Теперь выборка разделена: активные статусы отдаются целиком, каждая
    терминальная колонка — верхушкой в `TERMINAL_COLUMN_LIMIT` карточек, но с
    НАСТОЯЩИМ `count` из отдельного агрегата.
