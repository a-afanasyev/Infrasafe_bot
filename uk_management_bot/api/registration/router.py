...
# A9-P3-20: стабильные машинные коды 409 — фронт ветвится по ним, а не regex'ом
# по тексту detail. Код — в заголовке X-Error-Code, detail остаётся прежней
# строкой (обратная совместимость со старыми клиентами/бандлами SPA).
ERROR_CODE_HEADER = "X-Error-Code"
CODE_ALREADY_REGISTERED = "already_registered"
CODE_CONTACT_REQUIRED = "contact_required"
CODE_ALREADY_RESIDENT = "already_resident"
CODE_PREVIOUS_REJECTED = "previous_rejected"
CODE_ADDRESS_CONFLICT = "address_conflict"


def _conflict(code: str, detail: str) -> HTTPException:
    return HTTPException(
        status_code=status.HTTP_409_CONFLICT, detail=detail, headers={ERROR_CODE_HEADER: code}
    )
...
async def start(request: Request, body: StartIn, db: AsyncSession = Depends(get_db)):
    ...
    if existing and existing.status == "approved":
        raise _conflict(CODE_ALREADY_REGISTERED, "Уже зарегистрирован")
...
async def register_applicant(
    ...
    phone = existing.phone if existing else None
    if not phone:
        raise _conflict(CODE_CONTACT_REQUIRED, "Сначала поделитесь контактом в Telegram")
    ...
    if existing_ua is not None:
        ...
        if existing_ua.status == "approved":
            await db.rollback()
            raise _conflict(CODE_ALREADY_RESIDENT, "Вы уже подтверждены как житель этой квартиры")
        await db.rollback()
        raise _conflict(CODE_PREVIOUS_REJECTED, "Предыдущая заявка отклонена. Обратитесь к администратору.")

    try:
        await address_core.request_apartment(db, user_id=user.id, apartment_id=body.apartment_id)
    except AddressConflict as e:
        await db.rollback()
        raise _conflict(CODE_ADDRESS_CONFLICT, str(e))
